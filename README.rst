...
Salt Stack Formula to set up and configure varnish, a web application accelerator also known as a caching HTTP reverse proxy
...
Some states/ commands may refer to templates which aren't included in the files folder (``varnish/files``). Take a look at ``contrib/`` (if present) for e.g. template examples and place them in separate file roots (e.g. Git repository, refer to `GitFS <http://docs.saltstack.com/topics/tutorials/gitfs.html>`_) in your Salt master config.
